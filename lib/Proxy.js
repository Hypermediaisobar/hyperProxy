...
function Proxy(options) {
	...
	this.proxyRequest = function(request, response, reqURL) {
		var reqOptions = {
			...
			agent: false,//new https.Agent(reqOptions),
			auth: request.auth,
		...
		var tunnel;
		var proxyRequest;
		if (reqURL.proxy && reqURL.protocol === 'https:') {
			http.request({host: reqURL.proxy.hostname, port: reqURL.proxy.port, method: 'CONNECT', path: reqURL.host}).on('connect', function(res, socket, head){
				reqOptions.socket = socket;
				setupProxyRequest(https.request(reqOptions, handleResponse));
			}).end();
		}
		else {
			if (reqURL.proxy) {
				reqOptions.host = reqURL.proxy.hostname;
				reqOptions.port = reqURL.proxy.port;
				reqOptions.path = reqURL.protocol + '//' + reqURL.host + reqURL.path;
			}
			setupProxyRequest(http.request(reqOptions, handleResponse));
		}
	};
	...
	this.httpServer.addListener('connect', function(request, socket, head){
		// TODO: in theory tunnel can be used for HTTP-to-HTTP too, in which case we should not worry about certificates,
		//       and we should connect to options.httpPort instead of options.httpsPort.
		if (!options.useSNI || SNI.hasOwnProperty(request.headers['host'])) {
			self.createTunnel(true, socket, head);
			return;
		}

		// Create certificate before tunneling, so our httpServer can use it before client connects.
		pem.createCertificate({days: 1, selfSigned: true, organization: 'hyperProxy', commonName: request.headers['host'].replace(/:\d+$/, '')}, function(err, keys){
			if (err) {
				console.error(err);
			}
			else {
				SNI[request.headers['host']] = crypto.createCredentials({
					key: keys.serviceKey,
