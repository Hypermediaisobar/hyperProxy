...
function Proxy(options) {
	...
	this.serverSNIHandler = function(servername, callback) {
		if (SNI.hasOwnProperty(servername) && SNI[servername] !== true) {
			if (callback instanceof Function) {
				process.nextTick(callback.bind(this, null, SNI[servername].context));
				return;
			}
	...
	this.createCertificate = function(servername, callback) {
		...
		SNI[servername] = true;
		pem.createCertificate({days: 1, selfSigned: true, organization: 'hyperProxy', commonName: servername}, function(err, keys){
			if (err) {
				console.error(err);
			}
			else {
				SNI[servername] = crypto.createCredentials({
					key: keys.serviceKey,
	...
	this.proxyRequest = function(request, response, reqURL) {
		...
		if (reqURL.proxy) {
			var reqProxy = {
				...
				method: 'CONNECT',
				path: reqURL.hostname + ':' + reqURL.port,
			...
			http.request(reqProxy).on('connect', function(res, socket, head){
				...
				console.error('Proxy tunnel error', error, reqProxy, reqOptions);
			}).end();
		}
		else {
			setupProxyRequest((reqURL.protocol === 'https:' ? https : http).request(reqOptions, handleResponse));
