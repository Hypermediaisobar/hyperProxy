...
/**
 *	Wrapper for `createSecureContext`, so we can run safely on node v0.12 and v0.10
 *
 *	@see {@link http://nodejs.org/api/tls.html#tls_tls_createsecurecontext_details}
 *
 *	@private
 *	@function
 *	@param {Object} details
 */
var createSecureContext = tls.hasOwnProperty('createSecureContext') ? tls.createSecureContext.bind(tls) : crypto.createCredentials.bind(crypto);
...
function Proxy (options) {
	...
	options.cert = options.hasOwnProperty('cert') ? options.cert : null;
	...
	options.useSNI = options.hasOwnProperty('useSNI') ? options.useSNI : Boolean(pem);
	...
	this.initializeRootCertificate = function (callback) {
		if ((options.key === false && options.cert === false) || (options.key && options.cert)) {
	...
	this.createCertificate = function (servername, callback) {
		...
		pem.createCertificate({
			...
			if (err) {
				...
				SNI[servername] = createSecureContext({
					key : keys.clientKey,
	...
	this.proxyRequest = function (request, response, reqURL) {
		...
		util._extend(reqOptions.headers, request.headers);

		if (request.auth) {
			reqOptions.auth = request.auth;
		}

		if (reqOptions.headers.hasOwnProperty('proxy-connection')) {
			reqOptions.headers.connection = reqOptions.headers['proxy-connection'];
			delete reqOptions.headers['proxy-connection'];
		}
	...
	this.stop = function (callback) {
		...
		var onClosed = self.whenAllDone.bind(self, {id: 'closing', todo: 1 + (self.httpsServer ? 1 : 0), done: 0}, function () {
			...
			// Make sure we leave everything cleaned up
			ProxyAgent.destroy();
			callback();
		});
	...
	this.initialize = function (callback) {
		self.initializeRootCertificate(function () {
			...
			if (options.useSNI && options.key && options.cert) {
				SNI.localhost = createSecureContext({
					key : options.key,
					cert: options.cert
				});
			...
			if (self.httpsServer) {
				self.httpServer.addListener('connect', function (request, socket, head) {
					...
					var commonName = request.headers.hasOwnProperty('host') ? request.headers.host.replace(/:\d+$/, '') : target.hostname;
