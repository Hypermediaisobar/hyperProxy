 ...
 *
 *	@param {Object} overrides used by hyperProxy
 *	@param {Object} overrideProxy settings used by hyperProxy (with `http_port` specified)
 *	@param {Object} [defaultProxy] optional default proxy settings (with `port` specified)
 *	@returns {String}
 */
module.exports.script = function PACScript(overrides, overrideProxy, defaultProxy){
	'use strict';
 ...
 */
module.exports.handleRequest = function PACServeScript(request, response, script){
	'use strict';
 ...
 *	@returns {Object}
 */
module.exports.server = function PACServer(port, overrides, overrideProxy, defaultProxy) {
	'use strict';
	...
	self.port = port;
	self.script = module.exports.script;
	...
	console.log("\nServing PAC file for your web browser(s) on port "+self.port);
	console.log("\nTo test without possible additional problems with HTTPS certificates, you can start Chrome browser like this:\n\n---\n\t" + 'chrome --proxy-pac-url="http://127.0.0.1:'+self.port+'" --ignore-certificate-errors --user-data-dir=/tmp/random/unique' + "\n---\n\n");
};