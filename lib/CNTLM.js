...
module.exports = function(settings) {
	...
	var logfile = false;
	if (self.settings.verbose == 'debug') {
		logfile = path.join(process.cwd(), 'cntlm_'+Date.now()+'.log');
	}

	var areWeRunning = true;
	...
	self.spawn = function(){
		...
		var cntlmArgs = ['-v', '-l', self.settings.port, '-P', path.join(process.cwd(), 'cntlm.pid')];
		if (self.settings.verbose == 'debug') {
			console.log('CNTLM trace log: '+logfile);
			cntlmArgs.push('-T');
			cntlmArgs.push(logfile);
			cntlmArgs.push('-v');
			cntlmArgs.push('-s');
		}
		self._cntlm = spawn('cntlm', cntlmArgs, {cwd: self.settings.path, stdio: 'pipe'});
	/*
	...
	*/
		...
		self._cntlm.on('exit', function (code) {
			...
			if (self.settings.hitpoints <= 0) {
				setTimeout(function(){
					if (areWeRunning) process.exit(1);
				}, 100);
			}
			else {
				++self.settings.port;
