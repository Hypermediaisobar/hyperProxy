 ...
 *	@returns {Object}
 */
module.exports = function FilteredProxy(options) {
	'use strict';
	 ...
	 */
	self.getOwnHostnames = function(callback) {
		if (!self.IPs || options.domains) {
	 ...
	 *	@returns {boolean}
	 */
	self.isOwnHost = function(host, port) {
		var hostname;
	 ...
	 *
	 *	Filter function will be called with request, response and parsed URL object as arguments.
	 *
	 ...
	 */
	self.addFilter = function(name, filter) {
	...
	/*
	 *	Override `start()`, to make sure we have everything initialized before calling back.
	 *
	 *	@param {Function} callback
	 */
	 ...
	 *
	 *	FIXME: it does not check for too big sizes overall.
	 *	FIXME: it does not check for buffer "overflow" (it will throw exception when data is more than 8192 bytes long).
	 *
	 *	@private
	 *	@param {Object} message can be either from `request` or `response` event
	 *	@param {Function} callback
	 */
	self.drainMessage = function(message, callback) {
		...
		if (message.headers.hasOwnProperty('content-length')) {
			size = parseInt(message.headers['content-length']);
			if (isNaN(size)) {
				size = 8192;
			}
		}
	 ...
	 */
	self.on('request', function(request, response, reqURL){
		if (filtersCount < 1) {
			return;
		}
 ...
 */
util.inherits(module.exports, Proxy);