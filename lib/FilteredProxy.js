	...
	self.drainMessage = function(message, callback) {
		...
		if (message.headers.hasOwnProperty('content-length')) {
			...
			if (size > options.filtered.maxDrainBufferSize) {
				console.error('Tried to drain message bigger than '+options.filtered.maxDrainBufferSize);
				process.nextTick(callback);
				return;
			}
		}
		...
		message.on('data', function(data){
			...
			if (sum > options.filtered.maxDrainBufferSize || sum + options.filtered.drainBufferSize > options.filtered.maxDrainBufferSize) {
				console.error('Tried to drain '+sum+' bytes, which is morre than '+options.filtered.maxDrainBufferSize);
				return;
			}
