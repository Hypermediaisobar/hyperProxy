...
function NTLM () {
	...
	this.ntlm_sr_response = function (message2, credentials) {
		...
		result.ntlm = Buffer.concat([out1, out2, out3], out1.length + out2.length + out3.length);
	...
	this.createType2Message = function (targetName, flags, challenge, context, targetInfo, version) {
		...
		// Flags
		result.writeUInt32LE(Number(flags), 20);
		// Challenge
	...
	this.readType2Message = function (message2) {
		...
		if (result.targetNameBuffer.offset >= 56) {
			result.version = new self.OSVersion(data.readUInt8(48), data.readUInt8(49), data.readUInt16LE(50), data.readUInt32LE(52));
		}
	...
	this.createType3Message = function (message2, credentials) {
		...
		if (self.clientVersion > 1) {
			...
			// Flags
			result.writeUInt32LE(Number(message2.flags), 52 + (self.clientVersion > 1 ? 8 : 0));
		}
	...
	this.readType3Message = function (message3) {
		...
		if (minOffset > 64) {
			...
			if (result.version) {
				result.version = new self.OSVersion(
	 ...
	 *
	 *	@param {integer}	[unixTimestamp] optional UNIX timestamp, if not provided, Date.now() will be used
	 *	@returns {Buffer}
	 */
	this.createTimestamp = function (unixTimestamp) {
		if (!unixTimestamp) {
			unixTimestamp = Math.round(Date.now() / 1000);
		}

		// FIXME: this may create number larger than max allowed by JavaScript
		return self.createUInt64LE((unixTimestamp + 11644473600) * 10000000);
	};
	...
	this.credentials = function (username, domain, password, workstation) {
		...
		this.version = new self.OSVersion();
